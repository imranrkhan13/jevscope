...
No field list? Pass no fields (`fields: null` / `None`). A resume is read section by section with no AI key: name, email, phone, location, links, summary, each job (title, company, location, dates, one field per bullet), each project (name, tagline, tech, dates, one field per bullet), each degree (degree, field, school, location, dates), every skills line and any other section, with wrapped lines joined back. Dates can sit on the right, on their own line or first; "Company, Title", "Title, Company – Place" and two-line layouts all work; page footers, "Last updated" lines and "4 months" durations are skipped. Each field carries a description naming its job or project, which goes into the Jev question. Tested on 2 synthetic resumes and the RenderCV example CV in 3 layouts (spec/resumes.json). It never computes values the resume does not state, such as total years of experience. Receipts, bank statements and forms get the same no-key treatment. A store receipt gives the merchant and date when shown, one field group per item (name, quantity, unit price, price) and every labelled total (subtotal, tax, total, cash, change, TUNAI / KEMBALI). A bank statement gives the header (bank, account holder, account number, IFSC, branch, period, opening and closing balance) and one group per transaction row (date, description, DR/CR, amount, balance); wrapped or split rows are skipped rather than guessed. A form gives one field per filled blank ("NAME: ...") and keeps empty blanks as null values, never invented. Tested on real public data: 6 receipts from CORD v2 (CC BY 4.0), 3 forms from FUNSD (research use) and 3 bank statements from AgamiAI's Indian-Bank-Statements (Apache 2.0) — the bank statement set is SYNTHETIC (machine-generated), since real bank statements are not openly licensed (spec/receipts.json, spec/bank_statements.json, spec/forms.json). For other text: JevAPI finds every labelled field in the text ("Invoice No: ...", "Total 48,200") and Jev checks each one. On invoice-like text it also always asks about the vendor and the currency, even when no line labels them: it offers Jev the letterhead, "Remit to / Checks payable to" names and company-looking lines as options, plus a list of currencies, and Jev picks one or says not_stated. That finder is simple pattern matching; for free-form documents use the hosted `/api/v1/extract` without `fields`, which asks your AI for every field, with `"judge": "jev"`.
